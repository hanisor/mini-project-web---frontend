...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $requiredFields = ['Name', 'Description', 'Link'];
        ...
        if (isset($_SESSION["loggedAdminEmail"])) {
            ...
            if (mysqli_num_rows($resultAdminId) > 0) {
                ...
                $stmt = $con->prepare("INSERT INTO exercise (`Name`, `Description`, `Link`, `AdminId`, `GoalId`) VALUES (?, ?, ?, ?, ?)");
                $stmt->bind_param("ssssi", $jsonbody->Name, $jsonbody->Description, $jsonbody->Link, $adminId, $jsonbody->GoalId);
                $stmt->execute();

                // Return success response
                http_response_code(200);
                $response = array('success' => true, 'message' => 'Exercise successfully added');
                echo json_encode($response);
            } else {
                http_response_code(400);
        ...
        if (isset($_SESSION["loggedAdminEmail"])) {
            ...
            if (mysqli_num_rows($resultAdminId) > 0) {
                ...
                if (isset($requestData["Link"])) {
                    $setClauses[] = "Link = '" . $requestData["Link"] . "'";
                }
